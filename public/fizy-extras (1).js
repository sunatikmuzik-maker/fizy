...
function mount() {
  ...
  // Если на сайте уже есть своя вкладка «Калькулятор» — скрываем её,
  // чтобы не было двух кнопок. Остаётся версия с исправленным расчётом.
  const oldCalcTabs = [...nav.querySelectorAll('button')]
    .filter(b => /калькулятор/i.test(b.textContent || '') && !b.classList.contains('fx-tab'));
  oldCalcTabs.forEach(b => {
    const view = b.dataset.view;
    if (view) { const s = $('#' + view); if (s) s.hidden = true }
    b.remove();
  });
